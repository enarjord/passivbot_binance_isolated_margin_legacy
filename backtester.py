    ...
    for row in df.itertuples():
        ...
        for s in symbols:
            ...
            if getattr(row, lows[s]) <= entry_bid[s]:
                # long buy
                buy_cost = cost * max(1.0, min(5.0, (exit_ask[s] / getattr(row, means[s]))**15))
            ...
            if getattr(row, highs[s]) >= entry_ask[s]:
                # shrt sel
                sel_cost = cost * max(1.0, min(5.0, (getattr(row, means[s]) / exit_bid[s])**15))
            ...
            if long_cost[s] > min_exit_cost:
                ...
                if getattr(row, highs[s]) >= exit_ask[s]:
                    if balance[s2c[s]] >= long_amount[s]:
                        # long sel normal
                        long_exits[s].append({'price': exit_ask[s], 'amount': long_amount[s],
                                              'timestamp': row.Index})
                        quot_acquired = long_amount[s] * exit_ask[s]
                        balance[s2c[s]] -= long_amount[s]
            ...
            if shrt_cost[s] > min_exit_cost:
                ...
                if getattr(row, lows[s]) <= exit_bid[s]:
                    ...
                    if balance[quot] >= shrt_buy_cost:
                        # shrt buy normal
                        shrt_exits[s].append({'price': exit_bid[s], 'amount': shrt_amount[s],
                                              'timestamp': row.Index})
                        balance[quot] -= shrt_buy_cost
                        balance[s2c[s]] += shrt_amount[s] * fee
                        shrt_amount[s] = 0.0
                        shrt_cost[s] = 0.0
                        past_rolling_shrt_entries[s] = []
                    else:
